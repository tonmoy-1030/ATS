...
from .utils.confirmation_document import (
    confirm_appraisal_paper,
    confirmation_letter,
    extension_letter,
)
...
from .forms import (
    ...
    EmployeeConfirmationLetterFilter,
    PostingLetterFilter,
    TransferLetterFilter,
    AppointmentLetterFilter,
    JobOfferFilter,
...
def generate_reference_number():
    ...
    initial_interview_count = InterviewSchedule.objects.filter(
        interview_date__year=current_year
    ).count()
    final_interview_count = FinalInterviewSchedule.objects.filter(
        interview_date__year=current_year
    ).count()
...
def job_report(request, job_id):
    ...
    total_present = Candidate.objects.filter(
        job=job, attendance_status="Present"
    ).count()
    total_forwarded = Candidate.objects.filter(
        job=job, initial_interview_status="Forwarding for the next Interivew"
    ).count()
    ...
    percentage_present = (
        (total_present / total_candidates) * 100 if total_candidates > 0 else 0
    )
    percentage_forwarded = (
        (total_forwarded / total_candidates) * 100 if total_candidates > 0 else 0
    )
...
def job_report_document(request, job_id):
    ...
    placeholders = {
        ...
        "{INTERVIEWER}": "\n".join(
            f"{i+1}. {interviewer.name}" for i, interviewer in enumerate(interviewers)
        ),
    }
...
def Attendance_sheet(request, interview_id):
    ...
    candidate_list = Candidate.objects.filter(
        interview_schedule=initial_interview, invitation_status="Confirmed"
    )
    context = {"interview": initial_interview, "candidate_list": candidate_list}
...
def generate_candidate_assessment(request, interview_id):
    ...
    candidate_list = Candidate.objects.filter(
        interview_schedule=interview_schedule, attendance_status="Present"
    ).distinct()
    ...
    for candidate in candidate_list:
        ...
        if not hasattr(candidate, "details"):
            ...
            age = (
                str(timezone.now().year - candidate.details.date_of_birth.year)
                + " years"
            )
...
class OfferedCandidate(ListView):
    ...
    def get_queryset(self):
        return Candidate.objects.filter(
            offer__isnull=False, offer__joining_date__gte=timezone.now().date()
        )
...
def employee_list(request):
    employee_filter = EmployeeFilter(
        request.GET,
        queryset=Employee.objects.filter(details__isnull=False).order_by("-DOJ"),
    )
    ...
    return render(
        request,
        "documents/employee_joining_form.html",
        {"filter": employee_filter, "page_obj": page_obj},
    )


def JobOfferList(request):
    employee_filter = JobOfferFilter(
        request.GET,
        queryset=Offer.objects.filter(
            joining_date__gte=timezone.now().date(), offer_status="accepted"
        ),
    )
    ...
    return render(
        request,
        "documents/offered_candidates.html",
        {"filter": employee_filter, "page_obj": page_obj},
    )


def employee_confirmation_list(request):
    employee_filter = EmployeeConfirmationFilter(
        request.GET, queryset=Employee.objects.filter(active_status=True).order_by("id")
    )
    ...
    return render(
        request,
        "documents/employee_confirmation_list.html",
        {"filter": employee_filter, "page_obj": page_obj},
    )
...
def employee_confirmation_letter_list(request):
    employee_filter = EmployeeConfirmationLetterFilter(
        request.GET,
        queryset=EmployeeConfirmation.objects.all().order_by("employee__unit"),
    )
    paginator = Paginator(employee_filter.qs, 10)
    page_number = request.GET.get("page")
    try:
    ...
    return render(
        request,
        "documents/employee_confirmation_letter_list.html",
        {"filter": employee_filter, "page_obj": page_obj},
    )
...
def joining_form_generator(request):
    ...
    benevolent_fund = os.path.join(
        project_root, "documents/utils/MM. Benevolent Fund Application Form.pdf"
    )
    ...
    if request.method == "POST":
        ...
        for employee in employee_list:
            if not hasattr(employee, "details"):
                return HttpResponse(
                    f"{employee.name} has no details information available, please update information and try again!"
                )
            else:
                ...
                merger.append(buffer)
                if (
                    employee.unit.name != "Prime Pusti Limited"
                    and employee.unit.name != "Prime Cosmetics Limited"
                ):
...
def confirm_appraisal_form(request):
    ...
    response["Content-Disposition"] = (
        'inline; filename="confirmation_appraisal_paper.pdf"'
    )
    return response
...
def transfer_letter_list(request):
    employee_filter = TransferLetterFilter(
        request.GET, queryset=TransferOrder.objects.all().order_by("issue_date")
    )
    paginator = Paginator(employee_filter.qs, 10)
    page_number = request.GET.get("page")
    try:
    ...
    return render(
        request,
        "documents/transfer_order_list.html",
        {"filter": employee_filter, "page_obj": page_obj},
    )


def posting_letter_list(request):
    employee_filter = PostingLetterFilter(
        request.GET, queryset=PostingOrder.objects.all().order_by("issue_date")
    )
    paginator = Paginator(employee_filter.qs, 10)
    page_number = request.GET.get("page")
    try:
    ...
    return render(
        request,
        "documents/posting_order_list.html",
        {"filter": employee_filter, "page_obj": page_obj},
    )
...
def appointment_letter_list(request):
    ...
    return render(
        request, "documents/employee_appointment_letter.html", context=context
    )
...
def employee_list_envelope(request):
    employee_filter = EmployeeFilter(
        request.GET, queryset=Employee.objects.all().order_by("id")
    )
    ...
    return render(
        request,
        "documents/employee_envelope_form.html",
        {"filter": employee_filter, "page_obj": page_obj},
    )
...
def employee_list_name_tag(request):
    ...
    return render(
        request,
        "documents/employee_name_tag_form.html",
        {"filter": employee_filter, "page_obj": page_obj},
    )
...
def candidateReport(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not from_date or not to_date:
                return render(
                    request,
                    "documents/candidateDetails.html",
                    {"form": form, "error": "Please provide both from and to dates."},
                )
            ...
            with connection.cursor() as cursor:
                ...
                if results != ():
                    ...
                    output = Exporter().ExcelExporter(
                        title="Candidate list", headers=headers, body=results
                    )
                    output.seek(0)

                    response = HttpResponse(
                        output, content_type="application/vnd.ms-excel"
                    )
                    response["Content-Disposition"] = (
                        'attachment; filename="Candidate_Report.xlsx"'
                    )
                    return response
                else:
    ...
    return render(
        request,
        template_name="documents/reportCandidateDetails.html",
        context={"form": form},
    )
...
LEFT join jobs_businessunit on employees_employee.unit_id = jobs_businessunit.id;
    ...
    with connection.cursor() as cursor:
        ...
        if results != ():
            headers = [d[0] for d in description]
            output = Exporter().ExcelExporter(
                title="Employee List", headers=headers, body=results
            )
            output.seek(0)
            response = HttpResponse(output, content_type="application/vnd.ms-excel")
            response["content-Disposition"] = (
                'attachment; filename="Employee_Details.xlsx"'
            )
            return response
        else:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            with connection.cursor() as cursor:
                ...
                data_wrapped = [
                    [
                        Paragraph(cell, styleN) if isinstance(cell, str) else cell
                        for cell in row
                    ]
                    for row in data
                ]
                    ...
                    TableStyle(
                        [
                            ...
                            (
                                ...
                                (-1, -1),
                                11,
                            ),  # Regular font size for body rows
                ...
                response["Content-Disposition"] = (
                    'inline; filename="Vacancy_Report.pdf"'
                )
                return response
        else:
...
def auto_span_units_and_departments(pivot_table):
    ...
    for i in range(1, len(unit_col)):
        if unit_col[i] != unit_col[i - 1]:
            unit_span_styles.append(
                ("SPAN", (0, start_unit_idx), (0, i))
            )  # Span Unit column
            start_unit_idx = i + 1
    unit_span_styles.append(
        ("SPAN", (0, start_unit_idx), (0, len(unit_col)))
    )  # Span last group for Unit
    ...
    for i in range(1, len(department_col)):
        if department_col[i] != department_col[i - 1] or unit_col[i] != unit_col[i - 1]:
            department_span_styles.append(
                ("SPAN", (1, start_dept_idx), (1, i))
            )  # Span Department column
            start_dept_idx = i + 1
    department_span_styles.append(
        ("SPAN", (1, start_dept_idx), (1, len(department_col)))
